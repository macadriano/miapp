#!/usr/bin/env python
"""
Script para emular un recorrido realista entre Fitz Roy 6185, Isidro Casanova 
y Italia 1157, Burzaco con posiciones cada 100 metros aproximadamente.
"""
...
from gps.models import Movil, Posicion, MovilStatus, MovilGeocode
...
def generar_recorrido_realista():
    """Genera un recorrido realista entre las dos ubicaciones"""
    ...
    tiempo_total_minutos = 60  # 1 hora como mencionaste
    ...
    print(f"🚙 Usando móvil: {movil.patente} - {movil.alias}")
    ...
    if len(velocidades) < len(coordenadas):
        # Repetir el último valor si hay menos velocidades que posiciones
        velocidades.extend([velocidades[-1]] * (len(coordenadas) - len(velocidades)))
    elif len(velocidades) > len(coordenadas):
        # Tomar solo las velocidades necesarias
    ...
    for i, (lat, lon) in enumerate(coordenadas):
        ...
        posicion = Posicion.objects.create(
            empresa_id=1,  # Asumir empresa ID 1
        ...
        if i % 10 == 0:  # Mostrar progreso cada 10 posiciones
    ...
    if ultima_posicion:
        ...
        print(f"✅ MovilStatus actualizado para {movil.alias}")
    ...
    print(f"✅ MovilGeocode actualizado para {movil.alias}")
...
if __name__ == '__main__':
    try:
        posiciones_creadas = generar_recorrido_realista()
